  ...
  for (int i = 0; i < map->numberLayers; i++) {
    ...
    for (int k = 0; k < ld->amountOfTiles; k++) {
      printf("SourceX:%f\tSourceY:%f\tTargetX:%f\tTargetY:%f\n",
             (ld->tileData + k)->sourceX, (ld->tileData + k)->sourceY,
             (ld->tileData + k)->targetX, (ld->tileData + k)->targetY);
    }
  }
}
...
void renderLayer(TileMap *map, const char *layerName, Vector2 pos, float zoom) {
  ...
  for (int i = 0; i < ld->amountOfTiles; i++) {
    ...
    DrawTexturePro(
        ...
        (Rectangle){(td->targetX * zoom) + pos.x, (td->targetY * zoom) + pos.y,
                    ld->tileSize * zoom, ld->tileSize * zoom},
...
Rectangle *createCollisionData(TileMap *map, Vector2 pos, float zoom,
                               int *amount, errTileMap *err) {
  struct LayerData *ld;
  *amount = 0;
  ...
  for (int i = 0; i < map->numberLayers; i++) {
    ...
    if (ld->isCollisionLayer == true) {
      for (int k = 0; k < ld->amountOfTiles; k++, index++) {
        ...
        (colData + index)->width = (colData + index)->height =
            ld->tileSize * zoom;
      }
    }
  }
...
Rectangle *createCollisionDataLayer(TileMap *map, int layer, Vector2 pos,
                                    float zoom, int *amount, errTileMap *err) {
  struct LayerData *ld;
  *amount = 0;
...
void unloadCollisionData(Rectangle *col) { free(col); }
